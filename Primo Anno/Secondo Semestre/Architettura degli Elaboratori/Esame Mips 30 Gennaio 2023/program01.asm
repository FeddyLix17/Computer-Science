...
PrepareForPrintOutput:
    ...
    bge $t1, $zero, CheckLastSequence 
    move $t5, $zero     # resetto l'indice dell'array output
    li $v0, 1
    j PrintOutput

CheckLastSequence:
    # se sono qui dentro vuol dire che l'ultima sequenza di 3 bit non è completa
    # quindi devo allungarla aggiungendo 0 a destra
    # ma prima setto su vero il fatto che la stringa di input sia stata tradotta allungando l'ultima sequenza di 3 bit
    addi $t6, $t6, 1

    # setto gli ultimi 2 bit a 0 in caso essi siano numeri negativi
    bne $t2, SetCentralBitTo0
    bne $t3, SetLessSignificantBitTo0
    j PrintOutput

SetCentralBitTo0:
    move $t2, $zero
    j SetLessSignificantBitTo0

SetLessSignificantBitTo0:
    move $t3, $zero
    beq $t1, 1, Add4ToResult
    beq $t2, 1, Add2ToResult
    j SaveResultToArray
